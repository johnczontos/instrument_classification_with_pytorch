...
class Trainer:
    def __init__(self, model, criterion, optimizer, params, run):
        self.model = model
    ...
    def train(self, train_loader, pbar=None):
        ...
        total = 0
        correct = 0
        for i, (_, waveform, labels) in enumerate(train_loader):
            # Forward pass
    ...
    def validate(self, val_loader):
        ...
        correct = 0
        with torch.no_grad():
            for i, (_, waveform, labels) in enumerate(val_loader):
                # Forward pass
    ...
    def evaluate(self, data_loader):
        ...
        with torch.no_grad():
            for _, waveform, labels in data_loader:
                outputs = self.model(waveform)
